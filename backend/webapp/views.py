...
def encode_single_face(rgb):
    class DummyResult:
        status = "SUCCESS"
        encoding = [0.0] * 128  # dummy encoding
    return DummyResult()

def match_face(enc, known, ids, threshold=0.55):
    return None
...
def enroll_face_api(request):
    ...
    try:
        ...
        encoding_value = enc_result.encoding
        if isinstance(encoding_value, np.ndarray):
            encoding_value = encoding_value.tolist()
